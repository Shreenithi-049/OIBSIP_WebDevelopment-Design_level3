...
const HeroBanner = () => (
  <section className="relative overflow-hidden bg-gradient-to-br from-app-text via-primary to-secondary rounded-3xl mx-4 my-6 p-8 md:p-14 text-white">
    ...
    <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
      className="absolute -right-10 -top-10 text-[180px] opacity-20 select-none">🍕</motion.div>
    <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 3, repeat: Infinity }}
      className="absolute right-20 bottom-4 text-6xl opacity-40 select-none">🍕</motion.div>
  </section>
);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-4 my-8">
        {[
          ...
          <motion.div key={f.title} whileHover={{ y: -4 }} className="card p-4 text-center">
      ...
      <section id="menu" className="max-w-7xl mx-auto px-4 pb-16">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h2 className="text-3xl font-extrabold text-app-text">Our Menu 🍕</h2>
            <p className="text-gray-500 mt-1">Choose from our handcrafted selection</p>
          </div>
          <input
            type="text"
        ...
        <div className="flex gap-3 mb-8 flex-wrap">
          {categories.map((cat) => (
            ...
            >
              {cat === 'all' ? '🍕 All' : cat === 'veg' ? '🟢 Veg' : cat === 'non-veg' ? '🔴 Non-Veg' : '🌿 Vegan'}
            </motion.button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-app-text mb-2">No pizzas found</h3>
